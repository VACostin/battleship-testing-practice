...
const BOARD_SIZE = 10;
const ITERATION_NUMBER = 2;
const gameboard = Gameboard();
for (let i = 1; i <= ITERATION_NUMBER; i += 1) {
  const testString =
    i === 1
      ? ""
      : `Resetting board and executing tests again.\n`;
  describe(`${testString}  Placing ships round ${i}`, () => {
    let inputArray;
    beforeAll(() => {
  ...
  describe(`Receiving attacks round ${i}`, () => {
    let inputArray;
    beforeAll(() => {
      inputArray = [
  ...
  describe(`Checking if all ships are sunk round ${i}`, () => {
    ...
    describe(`Receiving attacks after all ships are sunk round ${i}`, () => {
      const positions = [
        { x: -2, y: BOARD_SIZE },
        { x: 1, y: 4 },
        { x: 3, y: 0 },
